module Jammit
  ...
  class Packager
    ...
    def initialize
      ...
      @packages = {
        :css => create_packages(@config[:css], :css),
        :js  => create_packages(@config[:js], :js)
      }
    end
    ...
    # every request.
    def create_packages(config, asset_type)
      packages = {}
      fill_in_wild_card(config, Jammit.wildcard_substitution)
      recursive_lookup(config)
      sanitize_input(config, asset_type) # add prefix and .js/.css if needed
    ...
    # sanitize everything
    def sanitize_input(config, asset_type)
      config.each do |key, value_array|
        config[key] = value_array.map do |value|
          value += ".#{asset_type}" if !value.end_with?(".#{asset_type}")
          value = Jammit.javascript_prefix + value if (asset_type == :js and !value.start_with?(Jammit.javascript_prefix))
          value = Jammit.stylesheet_prefix + value if (asset_type == :css and !value.start_with?(Jammit.stylesheet_prefix))
          value
        end
      end
    end
    ...
    # recusively lookup and resolve names
    def recursive_lookup(config)
      config.each do |key, value|
        config[key] = value.map{|x| resolve_true_path(x, config)}.flatten
      end
      config
    end
      ...
      config.dup.each do |key, value|
        if key.to_s.include?("?")
          ...
          substitution_values.each do |sub|
            config[key.to_s.gsub("?", sub).to_sym] = value.map{|x| x.gsub("?", sub)}
          end
        end
      end
    end
